...
- A flexible preprocessing pipeline built using **MLBox**. Transformations include missing value imputation, categorical encoding and feature scaling. <br/>
...
- **`examples/`**: This directory contains example files for the titanic dataset. Three files are included: `smoke_test_regression_schema.json`, `smoke_test_regression_train.csv` and `smoke_test_regression_test.csv`. You can place these files in the `inputs/schema`, `inputs/data/training` and `inputs/data/testing` folders, respectively.
...
```json
{
  ...
  "est__max_depth": { "search": "choice", "space": [5, 10, 15, 20, null] },
  "est__n_estimators": { "search": "choice", "space": [50, 100, 150, 200] },
  "est__num_leaves": { "search": "choice", "space": [20, 30, 40] }
}
```
...
- **Type**: String
- **Description**: Defines the name of the field where predictions will be stored.
- **Example**: `"prediction_field_name": "prediction"`

#### `scoring`

- **Type**: String
- **Description**: Represents the metric used to evaluate the performance of the model. A negative prefix (e.g., "neg\_") indicates that a lower score is better.
- **Example**: `"scoring": "neg_root_mean_squared_error"`
- **Note**: This is using scikit-learn's naming convention for scoring metrics.

#### `algorithms`

- **Type**: List of Strings
- **Description**: Lists the machine learning algorithms to be considered during the optimization/search process.
- **Example**: `"algorithms": ["LightGBM", "RandomForest", "ExtraTrees"]`
- **Available Algorithms**: {“LightGBM”, “RandomForest”, “ExtraTrees”, “Tree”, “Bagging”, “AdaBoost” and “Linear”}.

#### `est__max_depth`

- **Type**: Object (Dictionary)
- **Description**: Specifies the search space for the `max_depth` hyperparameter of the estimator.
  - **search**: Specifies the type of search. "choice" means selecting from a list of values.
  - **space**: Lists the possible values for `max_depth`.
- **Example**: `"est__max_depth": {"search": "choice", "space": [5, 10, 15, 20, null]}`
- **Note**: A value of `null` typically represents no maximum depth (i.e., nodes are expanded until all leaves are pure).

#### `est__n_estimators`

- **Type**: Object (Dictionary)
- **Description**: Specifies the search space for the `n_estimators` hyperparameter of the estimator, which represents the number of trees in the forest.
- **Example**: `"est__n_estimators": {"search": "choice", "space": [50, 100, 150, 200]}"`

#### `est__num_leaves`

- **Type**: Object (Dictionary)
- **Description**: Specifies the search space for the `num_leaves` hyperparameter, which is specific to the LightGBM algorithm. It defines the maximum number of leaves for the base learners.
- **Example**: `"est__num_leaves": {"search": "choice", "space": [20, 30, 40]}"`

For more information please visit: [MLBox docs](https://mlbox.readthedocs.io/en/latest/installation.html)
